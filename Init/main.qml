...
Rectangle {
    ...
    function loadUSBDirs(){
        qmlDirsModel.clear();
        var dirStr = panelController.usbDirs();
        if(dirStr.length !== 0){
    ...
    function loadLocalDirs(){
        qmlDirsModel.clear();
        var dirStr = panelController.localUIDirs();
        if(dirStr.length !== 0){
            var dirs = JSON.parse(dirStr);
            for(var i = 0; i < dirs.length; ++i){
                qmlDirsModel.append({"dirName":dirs[i]});
...
//        visible: false
    ...
    Column{
        ...
        ICButton{
            ...
            width: 150
            onButtonClicked: {
                panelController.setToRunningUIPath(qmlDirsModel.get(qmlDirs.currentIndex).dirName)
            }
        }
    }
